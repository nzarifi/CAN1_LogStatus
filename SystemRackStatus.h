...
static const char * const MasterOperationState_names[] = {
	...
	[masterOpStateAllStringsDisconnected] = "All Strings Disconnected",																																						   
...
typedef enum
{
	...
	POWER_STATE_WAIT_ADC_RELIABLE_RESULT,
	POWER_STATE_WAIT_RACK_SYS_REV,	//find rack and master revisions
	POWER_STATE_ACTIVE,
...
typedef struct
{
	...
	//uint8_t ALIGNED_(1) spare2[1];//24 complement to 4 bytes
} MasterStateHandlerStatus_t;
...
typedef struct
{
	...
	//uint8_t ALIGNED_(1) spare1[1];//28 complement to 4 bytes
}ChargerStatus_t; //28
...
typedef struct
{
    ...
    uint16_t ALIGNED_(2) stringCurrentWarningFlag;  //added for Raymond regen and discharge warnings
...
typedef struct
{
    ...
    int32_t ALIGNED_(2) leakageVoltMeasurementResults[LEAK_VOLT_MEAS_SIZE + 1];//12+16=28
...
typedef struct
{
	uint16_t ALIGNED_(2) temperatureCriticalHighWarning[MAX_THERMISTORS_PER_CELL];//4
	//for LTC3300, start
	...
	uint8_t ALIGNED_(1) spare[2];//324, complement to 4 byte boundary
...
typedef struct
{
    ...
    PassiveBalancingStatus_t ALIGNED_(4) passiveBalancingStatus;//452+20 =472
    ContactorStatus_t ALIGNED_(4) negativeContactorStatus;//472+8=480
} BMSCtrlStatus_t;//480



typedef struct  // this is the 1992 byte datastructure passed in the 0x144 messages(249 8 byte messages)
{
    ...
    ChargerStatus_t chargerStatus;//40+28=68
    EWSStatus_t eWSStatus;//68+4=72
    BMSCtrlStatus_t bMSCtrlStatus[MAX_RACKS_FOR_FORKLIFT];//72+(480*4)=1992
} MasterMakerStatus_t;//1992
